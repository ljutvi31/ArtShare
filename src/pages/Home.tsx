...
export function Home() {
  ...
  const styleCounts = useMemo(() => {
    ...
    items.forEach((item) => (counts[item.style] = (counts[item.style] || 0) + 1))
    return counts
  }, [items])

  const totalLikes = useMemo(() => items.reduce((s, it) => s + it.likes, 0), [items])
    ...
    <div className='container'>
        ...
        <div className={styles.heroContent}>
          ...
          <div className={styles.heroStats}>
            <div className={styles.heroStat}>
              ...
              <span className={styles.heroStatLabel}>{lang === 'fr' ? 'Œuvres' : 'Artworks'}</span>
            </div>
            <div className={styles.heroStat}>
              <span className={styles.heroStatNumber}>{new Set(items.map((i) => i.artist)).size}</span>
              <span className={styles.heroStatLabel}>{lang === 'fr' ? 'Artistes' : 'Artists'}</span>
            </div>
            <div className={styles.heroStat}>
              <span className={styles.heroStatNumber}>{totalLikes}</span>
              <span className={styles.heroStatLabel}>{lang === 'fr' ? 'Likes' : 'Likes'}</span>
            </div>
          </div>
        </div>
      </section>

      {/* ⚠️ '.controls' passe en flex (plus de grid) */}
      ...
      {filtered.length === 0 ? (
        <div className={styles.empty}>
          <h3 className={styles.emptyTitle}>{lang === 'fr' ? 'Aucune œuvre trouvée' : 'No artworks found'}</h3>
          <p className={styles.emptyText}>{t.noResults}</p>
        </div>
      ) : (
        <ArtGrid items={filtered} setItems={setItems} />
      )}
    </div>
  )
}
